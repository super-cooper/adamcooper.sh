...
private object WordleState {
    ...
    val solutions: Flow<WordleDB.Solution>
        get() = flow {
            ...
            for (id in WordleState.count downTo knownSolutions.count()) {
                val date = WordleState.WORDLE_FIRST_DATE.plus(id, DateTimeUnit.DAY)
                WordleState.log.info("Solving new Wordle puzzle $id ($date)")
                val answer = WordleState.wordle.answer(date.toJavaLocalDate())
...
private fun wordleGuessSequence(solution: WordleDB.Solution) =
    sequenceOf(
            solution.guess1,
            solution.guess2,
            solution.guess3,
    ...
    for (guess in wordleGuessSequence(solution)) {
        ...
        div(classes = "wordleFormattedResultRow") {
            results.zip(guess.asIterable()).forEach { (result, char) ->
                when (result) {
                    Wordle.Letter.GREEN ->
                        div(classes = "wordleFormattedResultGreen") { +char.toString() }
                    Wordle.Letter.YELLOW ->
                        div(classes = "wordleFormattedResultYellow") { +char.toString() }
...
private fun wordleCopyText(solution: WordleDB.Solution): String =
    buildString {
            val guesses = wordleGuessSequence(solution)
            val count =
                solution.guess6?.let { if (it != solution.answer) "X" else null }
                    ?: guesses.count().toString()
...
fun HTML.wordleApp() {
    ...
    body {
        ...
        div(classes = "app") {
            h1 { +"Wordle Solver" }
            div(classes = "appIntroduction") { h3 { this.wordleDescription() } }
